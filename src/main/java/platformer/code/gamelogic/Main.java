...
public class Main extends GameBase implements PlayerDieListener, PlayerWinListener, ScreenTransitionListener{
	...
	public void init() {
		...
		try {
			levels[0] = LeveldataLoader.loadLeveldata("src/main/java/platformer/maps/gasTester.txt");
			levels[1] = LeveldataLoader.loadLeveldata("src/main/java/platformer/maps/gasTester.txt");
		} catch (Exception e) {
			e.printStackTrace();
		}
